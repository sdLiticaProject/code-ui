...
export const RegistrationForm: FunctionComponent = () => {
  ...
  const isRegisterSuccess = useSelector((state: any) => state.login.type) === REGISTER_SUCCESS;
  const isRegisterLoading = useSelector((state: any) => state.login.type) === REGISTER_LOADING;
  ...
  useEffect(() => {
    if (isRegisterSuccess) {
      loginUser(
        getValues("login"),
        getValues("password"),
      )(dispatch).then((action: { type: string }) => {
        if (action?.type === LOGIN_SUCCESS) {
          history.push(HOME);
        }
      });
    }
  }, [dispatch, getValues, history, isRegisterSuccess]);
